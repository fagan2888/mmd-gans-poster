...
\node (uclbox) [shift={(-.36\linewidth,-4cm)}, align=left] at (current page.north east) {
  \includegraphics[height=12cm]{UCL_Black_Landscape.pdf} };

\draw[fill=blackish](current page.north west) rectangle ([yshift=-12.175cm, xshift=2cm] uclbox.north west);

\node[shift={(10cm,-7cm)}] at (current page.north west) { \includegraphics[height=3.75cm]{Imperial_logo_white.pdf}}; 
\end{tikzpicture} 
% \begin{tikzpicture}[remember picture,overlay]
